...
def aggregate_experiment_results_task(run_root: Path) -> dict[str, str]:
    ...
    cmd = [
        "python",
        "src/scripts/aggregate_experiment_results.py",
        "--run-root",
        str(run_root),
    ]
    run_script(cmd, cwd=".", out_log=out_log)
    return {
        "aggregated_per_run_csv": str(run_root / "aggregated_per_run.csv"),
        "hypothesis_tests_csv": str(run_root / "hypothesis_tests.csv"),
        "log": str(out_log),
    }


@flow(name="ad-screening-matrix-run")
...
def run_flow(
    ...
    if aggregate_results:
        aggregate_experiment_results_task.submit(run_root=root_dir).result()
    return root_dir
...
def main() -> None:
    ...
    print(f"Wrote: {out / 'aggregated_per_run.csv'}")
    print(f"Wrote: {out / 'hypothesis_tests.csv'}")
